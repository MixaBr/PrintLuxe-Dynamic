...
interface Chunk {
    ...
    metadata: {
        ...
        chunk_number: number;
        device_model: string; // New field for precise filtering
    }
}
...
/**
 * Extracts the device model from a filename based on the convention "type_MODEL.extension".
 * @param filename The name of the file.
 * @returns The extracted device model string or "unknown".
 */
function extractModelFromFilename(filename: string): string {
    const dotIndex = filename.lastIndexOf('.');
    const cleanFilename = dotIndex > -1 ? filename.substring(0, dotIndex) : filename;
    
    const underscoreIndex = cleanFilename.indexOf('_');
    if (underscoreIndex > -1 && underscoreIndex < cleanFilename.length - 1) {
        return cleanFilename.substring(underscoreIndex + 1);
    }
 ...
 * @param deviceModel The model of the device this text pertains to.
 * @param chunkNumber The starting chunk number.
 * @returns An array of text chunks.
 */
function recursiveSplitText(text: string, filename: string, deviceModel: string, chunkNumber: number = 1): Chunk[] {
    const chunks: Chunk[] = [];
    ...
    if (paragraphs.length > 1) {
        ...
        for (const p of paragraphs) {
            const paragraphChunks = recursiveSplitText(p, filename, deviceModel, currentChunkNumber);
    ...
    if (sentences.length > 1) {
            ...
            if ((tempChunk + sentence).length > CHUNK_SIZE) {
                if (tempChunk) {
                     chunks.push({
                        ...
                        metadata: { source_filename: filename, chunk_number: currentChunkNumber++, device_model: deviceModel },
                    });
                }
                tempChunk = sentence;
            } else {
        ...
        for (const chunk of chunks) {
            const subChunks = recursiveSplitText(chunk.text_content, filename, deviceModel, finalChunkNumber);
 ...
 * @param deviceModel The model of the device this HTML pertains to.
 * @returns An array of HTML chunks.
 */
function splitHtmlIntoChunks(html: string, filename: string, deviceModel: string): Chunk[] {
...
export async function processAndEmbedFile(prevState: ActionResult, formData: FormData): Promise<ActionResult> {
    ...
    try {
        ...
        const deviceModel = extractModelFromFilename(file.name);
        let chunks: Chunk[];
        ...
        if (file.type === 'application/pdf') {
            ...
            chunks = recursiveSplitText(pdfData.text, file.name, deviceModel);
        } else if (file.type === 'text/html') {
            const htmlContent = fileBuffer.toString('utf-8');
            chunks = splitHtmlIntoChunks(htmlContent, file.name, deviceModel);
        } else {
        ...
        console.log(`Extracted device model "${deviceModel}" from ${file.name}. Starting to process ${chunks.length} chunks.`);
